...
function DemoPage() {
  ...
  return (
    <div className="demo-page" style={{ minHeight: '100vh', background: 'var(--sand)', display: 'flex', flexDirection: 'column' }}>
      <Header />

      <main className="page-container">
        <section className="page-header">
          <div className="page-header-text">
            <span className="intro-eyebrow" style={{ color: 'var(--warm-clay)' }}>Simulation Environment</span>
        ...
        <section style={{
          ...
          <div>
            ...
            <p style={{ fontSize: '0.84rem', color: 'var(--text-primary)', lineHeight: 1.45 }}>{demoDisclaimer}</p>
          </div>
        </section>
        ...
        <section className="wellness-card">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '14px' }}>
            ...
            <div style={{ padding: '16px 20px', background: 'var(--warning-bg)', borderRadius: 'var(--radius-sm)', border: '1px solid var(--warning-border)' }}>
              <span style={{ fontSize: '0.78rem', color: 'var(--warning)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.04em' }}>
                Moderate / High
              </span>
            ...
            <div style={{ padding: '16px 20px', background: 'var(--danger-bg)', borderRadius: 'var(--radius-sm)', border: '1px solid var(--danger-border)' }}>
              <span style={{ fontSize: '0.78rem', color: 'var(--danger)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.04em' }}>
                Urgent Attention
              </span>
              <div style={{ fontSize: '1.8rem', fontWeight: 700, color: 'var(--danger)', marginTop: '4px' }}>
              ...
              <tbody>
                {demoCases.map((caseItem) => (
                  <tr key={caseItem.caseId}>
                    <td style={{ fontWeight: 600, color: 'var(--text-primary)' }}>{caseItem.caseId}</td>
                    <td style={{ color: 'var(--text-body)' }}>{caseItem.caseType}</td>
                    <td>
                      <RiskBadge level={caseItem.riskLevel} size="small" />
                    </td>
                    <td style={{ fontWeight: 700, color: 'var(--text-primary)' }}>{caseItem.latestDistressScore}/100</td>
                    <td style={{ color: 'var(--text-secondary)', fontWeight: 500 }}>{caseItem.trend}</td>
